...
__all__=[
    'ForcePartLammps', 'write_lammps_table', 'write_lammps_data'
, 'BondedNeighborList'
]
...
class BondedNeighborList(NeighborList):
    ...
    '''
    def __init__(self, system, N_host_atoms, selected=None):
        '''
           **Arguments:**
        ...
        if selected is None:
            ...
            for bond in system.bonds:
                if bond[0] < N_host_atoms: bonded.append(bond)
            # Add all 1-3 neighbors
            for angle in system.iter_angles():
                if angle[0] < N_host_atoms: bonded.append(np.array([angle[0],angle[2]]))
...
#            print (xy+0.5*x)/x, (xz+0.5*x)/x, (yz+0.5*y)/y
...
def write_lammps_data(system, fn='lammps.data', triclinic=True):
    ...
    # Add bonds with ghost atom
    bonds = system.bonds.copy()

    for i, ffatype_id in enumerate(system.ffatype_ids):
        if system.ffatypes[ffatype_id]=='M':
            bonds = np.concatenate((bonds, [[i-3, i]]), axis=0)

    fdat = open(fn,'w')
    fdat.write("Generated by Yaff\n\n%20d atoms\n%20d bonds\n%20d angles \n%20d dihedrals\n%20d impropers\n\n" % (system.natom, len(bonds), 0, 0, 0))
